...
module.exports = React.createClass({
	getInitialState: function () {
		...
		let graphicState = _.has(viewBody.chart_conf, 'type');
		return {
			'body': viewBody,
			'dimension_new': false,
			'kpi_new': false,
			...
			'initialCreateGraphicState': _.has(viewBody.chart_conf, 'type')
		}
	},
	...
	viewChart: function (conf) {
		...
		const {viewBody} = this.props;
		document.getElementById('c1').innerHTML = null;
		let Fields =viewBody.table_conf.fields;
	...
	render: function () {
		const {viewBody} = this.props;
		let Fields = viewBody.table_conf.fields;
		let chartConf = _.has(viewBody.chart_conf, 'type') ? viewBody.chart_conf : false;
		let fields = Fields.dimension_fields.concat(Fields.data_fields);
		let dimension;
		let KPI;
		...
		return (
			<div className="view-body">
				...
				<ShowTableContent
					onTableConf={viewBody.table_conf}
					onTbody={viewBody.data}
				/>
			</div>
		)
	}
